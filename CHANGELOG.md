...
## v0.16.2 · 2026-09-16 · rubric 2.15.0-skill.1

Pre-launch housekeeping. No score moves.

- **The footer's AI notice says "as is".** The report already stated that it is generated
  with AI and can make mistakes; it now also says it is provided as is, without warranty,
  and that the scores point at what to improve rather than at rankings PinMeTo guarantees.
  That puts the `SKILL.md` "no ranking guarantees" rule in front of the reader.
- **LICENSE ships in the `.skill` payload.** The front matter said "See LICENSE" but
  `package-skill.sh` never packed it, so the vendored copy in `claude-plugins` had no
  license file.
- **`docs/rubric.md`, a readable rubric page**, generated by `scripts/render-rubric.mjs`
  from the rubric JSON and the Theme mapping, with one plain-language sentence per check.
  `check-references.mjs` fails when the page is stale or a rubric id has no sentence, so it
  moves with every rubric bump.
- **README refreshed**: current version, the plugin wiring described as done rather than as
  a to-do, the layout table covering `scripts/`, `tests/`, `docs/` and CI, an example
  report screenshot, and a link to the rubric page.
