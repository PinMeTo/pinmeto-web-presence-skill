...
11. **Footer**: one hairline, then a muted **AI notice** on its own line —
    "Generated with AI from automated checks of public signals. It can make mistakes;
    verify the findings before acting on them. Provided as is, without warranty: the
    scores show where to improve visibility, not rankings PinMeTo guarantees." — then the
    identity row: "Generated by the
    PinMeTo web presence skill" at the left and "Rubric <version>" at the right. All small and
    muted, no orange, no border. The notice supports transparency under the EU AI Act's
    Article 50 obligations for AI-generated content, so it renders on every report and prints
    with the rest of the footer — never behind the expander, never omitted.
