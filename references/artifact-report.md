...
11. **Footer**: one hairline, then a muted **AI notice** on its own line —
    ...
    verify the findings before acting on them." — then the identity row: "Generated by the
    PinMeTo web presence skill" at the left and "Rubric <version>" at the right. All small and
    muted, no orange, no border. The notice is a transparency requirement (EU AI Act Article 50:
    content generated by AI is disclosed as such), so it renders on every report and prints with
    the rest of the footer — never behind the expander, never omitted.
