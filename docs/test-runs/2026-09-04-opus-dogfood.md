...
| 2 | One `curl` script, 15 URLs in one concurrent burst: the homepage over https and over http, `/locations`, the five office pages, robots.txt, `/llms.txt`, `/llms-full.txt`, the three `.well-known` paths and the blog index. Eight more followed as they were discovered: the sitemap index and both child sitemaps, `/locator/`, a blog article, a product page, one skill file and `webmcp.js`, plus four `Accept: text/markdown` probes against URLs already in that list. 23 distinct URLs in total. PSI attempted on three of them first, in the background. Then the **rendered pass** in the browser on all five office pages plus both hubs |
