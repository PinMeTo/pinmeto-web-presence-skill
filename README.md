...
rubric (v2.10.0-skill.1, a skill-line fork of the v2.8.0 rubric in `pinmeto-www-reports` that re-adds Bing and adds a PinMeTo-connection check). It produces an updatable HTML report
...
2. Tag and push: `git tag v0.5.0 && git push --tags`.
...
```json
{
  "skills": {
    ...
    "pinmeto-web-presence":     { "repo": "PinMeTo/pinmeto-web-presence-skill",     "version": "0.5.0" }
  }
}
```
...
`v0.5.0` — four-pillar methodology on rubric 2.10.0-skill.1 (MLPR 2.8.0 + Bing re-added + PinMeTo-connection check); GEO evidence
