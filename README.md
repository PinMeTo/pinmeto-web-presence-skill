...
2. Tag and push: `git tag v0.11.0 && git push --tags`.
...
```json
{
  "skills": {
    ...
    "pinmeto-web-presence":     { "repo": "PinMeTo/pinmeto-web-presence-skill",     "version": "0.11.0" }
  }
}
```
...
`v0.11.0` — four-pillar methodology on rubric 2.14.0-skill.1 (MLPR 2.8.0 + Bing re-added +
PinMeTo-connection check); GEO evidence via real map surfaces in a browser (no Places API /
MapKit); host-native updatable report with embedded scan history. Refine against real audits
before v1.
